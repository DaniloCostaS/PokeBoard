    ...
    static handleRemoteAction(action: any) { 
        ...
        switch(action.type) { 
            ...
            case 'PVP_SYNC_DAMAGE': 
                ...
                if(targetP) {
                    // Atualiza dados locais (visual para quem está assistindo)
                    ...
                    // CORREÇÃO CRÍTICA: Se eu sou o alvo e fui resetado, executo a lógica de cura total
                    if(targetP.id === this.myPlayerId) {
                        if (action.payload.resetPos) {
                            // Derrota Total: Cura tudo e sincroniza o estado "Curado"
                            Game.handleTotalDefeat(targetP); 
                        } else {
                            // Dano Parcial: Apenas salva o estado atual
