...
export class Battle {
    ...
    static startRound(selectedMon: Pokemon) { const Network = (window as any).Network; document.getElementById('pkmn-select-modal')!.style.display = 'none'; this.active = true; this.activeMon = selectedMon; this.determineTurnOrder(); this.renderBattleScreen(); const enemyId = this.enemyPlayer ? this.enemyPlayer.id : -1; if(Network.isOnline && this.player!.id === Network.myPlayerId) { const npcImg = (this.opponent as any)._npcImage || ""; const npcName = (this.opponent as any)._npcName || ""; Network.sendAction('BATTLE_START', { pId: this.player!.id, monIdx: this.player!.team.indexOf(this.activeMon), oppData: this.opponent, isPvP: this.isPvP, reward: this.reward, enemyId, isGym: this.isGym, gymId: this.gymId, npcImage: npcImg, npcName: npcName, battleTitle: this.battleTitle }); } }
    static determineTurnOrder() { if (!this.activeMon || !this.opponent) return; if (this.activeEffects.stunOpponent && this.activeEffects.stunOpponent > 0) { this.isPlayerTurn = true; this.logBattle(`⚡ Inimigo atordoado! (${this.activeEffects.stunOpponent} turnos)`); } else { let playerGoesFirst = false; if (this.activeMon.speed > this.opponent.speed) playerGoesFirst = true; else if (this.activeMon.speed < this.opponent.speed) playerGoesFirst = false; else playerGoesFirst = Math.random() > 0.5; this.isPlayerTurn = playerGoesFirst; } this.processingAction = false; this.updateButtons(); this.updateUI(); if(!this.activeEffects.stunOpponent) { const faster = this.isPlayerTurn ? this.activeMon.name : this.opponent.name; this.logBattle(`💨 ${faster} age primeiro!`); } if (!this.isPlayerTurn) { this.processingAction = true; this.updateButtons(); setTimeout(() => this.enemyTurn(), 2000); } }
    static updateButtons() { const Network = (window as any).Network; const btns = document.querySelectorAll('.battle-actions button'); const isMyBattle = Network.isOnline ? (this.player && this.player.id === Network.myPlayerId) : true; const canAct = this.isPlayerTurn && !this.processingAction && isMyBattle; btns.forEach((btn: Element) => { (btn as HTMLButtonElement).disabled = !canAct; }); if(this.isPvP || this.isGym) { (document.getElementById('btn-run') as HTMLButtonElement).disabled = true; } }
    static startFromNetwork(payload: any) { const Game = (window as any).Game; const p = Game.players[payload.pId]; if (!p) return; this.active = true; this.player = p; this.activeMon = p.team[payload.monIdx]; this.opponent = new Pokemon(payload.oppData.id, payload.oppData.level, payload.oppData.isShiny); Object.assign(this.opponent, payload.oppData); this.isPvP = payload.isPvP; this.isGym = payload.isGym; this.gymId = payload.gymId; this.isNPC = (!payload.isPvP && payload.reward > 0); if(payload.enemyId >= 0) this.enemyPlayer = Game.players[payload.enemyId]; if(payload.npcImage) (this.opponent as any)._npcImage = payload.npcImage; if(payload.npcName) (this.opponent as any)._npcName = payload.npcName; if(payload.battleTitle) this.battleTitle = payload.battleTitle; this.renderBattleScreen(); }
    ...
    static attack() { const Network = (window as any).Network; if(!this.activeMon || !this.opponent) return; this.processingAction = true; this.updateButtons(); let calc = this.calculateDamage(this.activeMon, this.opponent, true); let dmg = calc.damage; this.opponent.currentHp = Math.max(0, this.opponent.currentHp - dmg); const logMsg = `${this.activeMon.name} atacou! 💥${dmg} ${calc.msg}`; this.logBattle(logMsg); this.updateUI(); if(Network.isOnline) Network.sendAction('BATTLE_UPDATE', { plyHp: this.activeMon.currentHp, oppHp: this.opponent.currentHp, msg: logMsg }); if(this.opponent.currentHp <= 0) { setTimeout(() => { this.checkWinCondition(); this.processingAction = false; }, 1000); } else { this.isPlayerTurn = false; this.updateButtons(); setTimeout(() => this.enemyTurn(), 2000); } }
    static enemyTurn() { const Network = (window as any).Network; if(!this.activeMon || !this.opponent) return; if (this.activeEffects.stunOpponent && this.activeEffects.stunOpponent > 0) { this.activeEffects.stunOpponent--; this.logBattle("Inimigo atordoado! Passou a vez."); this.isPlayerTurn = true; this.processingAction = false; this.updateButtons(); return; } this.processingAction = true; this.isPlayerTurn = false; let calc = this.calculateDamage(this.opponent, this.activeMon, false); let dmg = calc.damage; this.activeMon.currentHp = Math.max(0, this.activeMon.currentHp - dmg); const logMsg = `${this.opponent.name} atacou! 💥${dmg} ${calc.msg}`; this.logBattle(logMsg); this.updateUI(); if (this.activeEffects.counter && this.activeEffects.counter > 0) { const reflect = Math.floor(dmg * 0.5); if (reflect > 0) { this.opponent.currentHp = Math.max(0, this.opponent.currentHp - reflect); this.logBattle(`🔁 Contra-ataque! Inimigo sofreu ${reflect} de dano.`); this.activeEffects.counter--; this.updateUI(); } } if(Network.isOnline && this.player && this.player.id === Network.myPlayerId) { Network.sendAction('BATTLE_UPDATE', { plyHp: this.activeMon.currentHp, oppHp: this.opponent.currentHp, msg: logMsg }); } if(this.activeMon.currentHp <= 0) { setTimeout(() => { this.handleFaint(); }, 1000); } else { this.isPlayerTurn = true; this.processingAction = false; this.updateButtons(); } }
    ...
    static useItem(key: string, data: ItemData) {
        ...
        if (data.type === 'capture') {
            ...
            if (Network.isOnline) Network.sendAction('BATTLE_UPDATE', { plyHp: this.activeMon!.currentHp, oppHp: this.opponent!.currentHp, msg: `Usou ${data.name}!` });
