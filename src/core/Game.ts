...
export class Game {
    ...
    static async rollDice() { 
        ...
        if (this.forcedDiceValue > 0) {
            ...
            if (p.effects.slow && p.effects.slow > 0) {
                ...
                p.effects.slow--;
                this.log("🕸️ Lentidão! Rolou apenas 1d6.");
            } else {
    ...
    static debugMove() { 
        ...
        if(Network.isOnline) { Network.sendAction('ROLL', { result: result }); return; } 
    ...
    static async movePlayerLogic(steps: number, pId: number) {
        ...
        for(let i=0; i<steps; i++) {
            ...
            p.x = nextCoord.x; p.y = nextCoord.y;
    ...
    static handleCityChoice(c: string) { 
        if(c==='heal') { 
            this.getCurrentPlayer().team.forEach(p=>p.heal(999)); 
